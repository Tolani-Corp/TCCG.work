  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold text-slate-800">
          Project location
          <input value={location} onChange={(e) => setLocation(e.target.value)} className={inputClass} placeholder="City, state" />
        </label>
        <label className="text-sm font-semibold text-slate-800">
          Need-by / bid date
          <input value={needBy} onChange={(e) => setNeedBy(e.target.value)} className={inputClass} placeholder="Date or timeframe" />
        </label>
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        <button type="submit" className="rounded-lg bg-[var(--accent-primary)] px-5 py-3 text-sm font-semibold text-white">
          Prepare email to TCCG
        </button>
        <a href="mailto:info@tccg.work" className="text-sm font-semibold text-slate-700">
          Or email info@tccg.work directly
        </a>
      </div>
      <p className="mt-4 text-xs leading-5 text-slate-500">
        This form prepares an email in your device&apos;s email client. It does not create a server-side lead record, upload project files or prove that TCCG accepted the opportunity.
      </p>
    </form>
  );
}
